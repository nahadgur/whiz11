    ...
    <div className="max-w-7xl mx-auto px-6">
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm">
            {UK_CITIES.slice(0, 10).map((c) => (
              <li key={c.slug}><Link href={`/locations/${c.slug}`} className="hover:text-white transition-colors">{c.label}</Link></li>
            ))}
          </ul>
        </div>
        <div>
      ...
      <div className="pt-8 border-t border-slate-800">
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1.5">
          {UK_CITIES.map((c) => (
            <Link key={c.slug} href={`/locations/${c.slug}`} className="text-xs text-slate-600 hover:text-slate-300 transition-colors">{c.label}</Link>
          ))}
        </div>
      </div>
    </div>
