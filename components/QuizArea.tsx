...
import { ArrowRight, Loader2, Timer, HelpCircle, AlertCircle, BookOpen, CheckCircle, XCircle, ArrowUp, ArrowDown } from 'lucide-react';
...
export const QuizArea: React.FC<QuizAreaProps> = ({ subject, schoolType, mode, quizType, topic, onComplete, onAskTutor }) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const qs = await generateQuestions(subject, schoolType, loadTopic, quizType);
      if (mounted) {
  ...
  }, [subject, schoolType, mode, topic, quizType]);
  ...
  const handleTextSubmit = () => {
    ...
    // Normalize logic for comparison
    const normalizedInput = textInput.trim().toLowerCase();
    const normalizedCorrect = currentQ.correctAnswer.trim().toLowerCase();
    
    // Check if input is contained in answer or answer is contained in input (for leniency) or exact match
  ...
  const handleOrderSubmit = () => {
    ...
    const correctOrderString = currentQ.correctAnswer; // "A,B,C"
    
    // Compare string representations
    // We normalize spaces
    const isCorrect = userOrderString.replace(/\s/g, '') === correctOrderString.replace(/\s/g, '');
  ...
  return (
    <div className="max-w-3xl mx-auto p-3 sm:p-6 pb-24">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <div className="bg-white rounded-3xl p-5 sm:p-10 shadow-xl shadow-indigo-100 border border-white mb-6">
            <motion.h3 
            ...
            {(quizType === 'multiple-choice' || quizType === 'true-false') && (
                ...
                {currentQ.options.map((opt, idx) => {
                    ...
                    <motion.button
                        ...
                        className={`w-full text-left p-4 rounded-xl transition-colors duration-200 flex items-center gap-4 group ${containerClass} ${quizType === 'true-false' ? 'justify-center flex-col py-8' : ''}`}
                    >
                        {quizType !== 'true-false' && (
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-black transition-colors shrink-0 ${circleClass}`}>
                            {String.fromCharCode(65 + idx)}
                        </div>
                        )}
                        <span className={`font-semibold text-slate-700 flex-1 ${quizType === 'true-false' ? 'text-xl sm:text-2xl text-center' : 'text-base sm:text-lg'}`}>{opt}</span>
                ...
                <div className="space-y-3">
                    ...
                    {orderingItems.map((item, idx) => (
                        <motion.div 
                            key={item}
                            layout
                            className={`p-3 sm:p-4 rounded-xl border-2 flex items-center gap-3 sm:gap-4 bg-white ${state.selectedAnswer ? (state.isCorrect ? 'border-emerald-200 bg-emerald-50' : 'border-slate-200') : 'border-slate-100 hover:border-indigo-200'}`}
                        >
                    ...
                    {!state.selectedAnswer && (
                        <div className="pt-4">
                            <Button onClick={handleOrderSubmit} className="w-full justify-center">Check Order</Button>
                        </div>
                    )}
                </div>
            )}
          ...
          <div className="flex flex-col-reverse sm:flex-row justify-between items-center gap-4">
            ...
            {state.selectedAnswer && (
              ...
              >
                <Button onClick={nextQuestion} size="lg" className="px-10 shadow-lg shadow-indigo-300 w-full sm:w-auto justify-center">
                  {state.currentIndex === state.questions.length - 1 ? 'Finish Exam' : 'Next Question'} <ArrowRight size={20} />
                </Button>
              </motion.div>
            )}
            </AnimatePresence>
