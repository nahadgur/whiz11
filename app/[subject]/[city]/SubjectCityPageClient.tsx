...
import {
  ...
  CheckCircle, Trophy, Target, BookOpen, Users,
  Clock, Zap, Shield,
} from 'lucide-react';
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 pt-16 pb-24 px-4 sm:px-6">
        ...
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          ...
          <div className="absolute bottom-0 right-0 w-80 h-80 bg-violet-600 rounded-full blur-[100px] opacity-10" />
        </div>
        ...
        <div className="relative max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            {/* Left: copy */}
            <div>
              <div className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-sm font-bold mb-5 ${style.light} ${style.accent} border border-current/20`}>
                <MapPin size={14} />
                {city.label} · 11+ {subject.label}
              </div>

              <h1 className="text-4xl sm:text-5xl font-black text-white tracking-tight leading-[1.1] mb-5">
                Free 11+{' '}
                <span className={`text-transparent bg-clip-text bg-gradient-to-r ${style.gradient}`}>
                  {subject.label}
                </span>{' '}
                Practice in {city.label}
              </h1>
              ...
              <div className="flex flex-wrap gap-3 mb-8">
                {[
                  { icon: Shield, text: 'GL & CEM aligned' },
                  { icon: Zap, text: 'Instant feedback' },
                  { icon: Clock, text: 'Timed practice' },
                ].map((t) => (
              ...
              <button
                ...
                className="group inline-flex items-center gap-3 px-8 py-4 bg-white text-slate-900 font-black text-lg rounded-2xl shadow-2xl hover:scale-105 hover:shadow-white/20 transition-all"
              >
                Start free practice
        ...
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-3xl sm:text-4xl font-black text-slate-900 mb-3">
              {subject.label} topics in your {city.label} 11+ exam
            </h2>
            <p className="text-slate-500 text-lg max-w-xl mx-auto">
              Every topic your child needs to master, with questions at exactly the right difficulty level.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
            {topics.map((topic, i) => (
              <motion.button
                ...
                onClick={() => setShowModal(true)}
                className="flex items-center gap-3 p-4 rounded-2xl bg-slate-50 border border-slate-100 hover:border-indigo-200 hover:bg-indigo-50 hover:shadow-lg transition-all group text-left"
              >
                <div className={`w-7 h-7 rounded-lg ${style.light} flex items-center justify-center shrink-0`}>
        ...
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-black text-slate-900 mb-3">
              How {city.label} families use WhizPrep
            </h2>
          </div>
      ...
      <section className="py-24 px-4 sm:px-6 bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 relative overflow-hidden">
        ...
        <div className="relative max-w-3xl mx-auto text-center">
          <h2 className="text-4xl sm:text-6xl font-black text-white tracking-tight mb-6 leading-tight">
            Give your child the best chance at {city.label}'s top schools.
          </h2>
          <p className="text-slate-300 text-xl mb-10 max-w-xl mx-auto">
            Free 11+ {subject.label} practice. No card needed. Start in under a minute.
          </p>
          <button
            onClick={() => setShowModal(true)}
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {allCities.filter((c) => c.slug !== city.slug).slice(0, 10).map((c) => (
                  ...
                  >
                    <MapPin size={11} className="text-slate-300 group-hover:text-indigo-400 shrink-0" />
                    {c.label}
                  </Link>
                ))}
              </div>
