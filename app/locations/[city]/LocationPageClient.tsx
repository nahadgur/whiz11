...
import {
  ...
  CheckCircle, School, Building2, BookOpen,
} from 'lucide-react';
...
const CITY_CONTENT: Record<string, {
  ...
  default: {
    blurb: 'Families across the UK are competing for places at grammar and independent schools. WhizPrep gives your child the best possible preparation with exam-style questions, instant feedback and progress tracking.',
  },
};
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-950 pt-20 pb-28 px-4 sm:px-6">
        <div className="absolute inset-0 pointer-events-none">
          ...
          <div className="absolute bottom-0 left-0 w-96 h-96 bg-violet-600 rounded-full blur-[100px] opacity-10" />
        </div>
      ...
      <section className="py-20 px-4 sm:px-6 bg-white">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-3xl sm:text-4xl font-black text-slate-900 mb-3">
              11+ subjects for {city.label} students
            </h2>
            ...
            {subjects.map((s, i) => (
              ...
              >
                <Link
                  ...
                  className="flex items-center gap-4 p-5 bg-white rounded-2xl border border-slate-100 hover:border-indigo-200 hover:shadow-xl hover:shadow-indigo-50 transition-all group"
                >
      ...
      <section className="py-16 px-4 sm:px-6 bg-gradient-to-r from-indigo-600 to-violet-600">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-indigo-100 text-lg mb-8">
            Hundreds of practice questions. Instant feedback. No card needed.
          </p>
          <button
            onClick={() => setShowModal(true)}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-2">
            {allCities.filter((c) => c.slug !== city.slug).map((c) => (
              <Link
                ...
                className="flex items-center gap-2 px-3 py-2.5 bg-white rounded-xl border border-slate-100 hover:border-indigo-200 hover:bg-indigo-50 text-slate-600 hover:text-indigo-700 text-sm font-semibold transition-all group"
              >
                <MapPin size={12} className="text-slate-300 group-hover:text-indigo-400 shrink-0" />
                {c.label}
              </Link>
            ))}
          </div>
